...
Jeśli mamy bazę danych dotyczącą np. filmów, chcemy mieć możliwość 
robienia z naszymi filmami kilku podstawowych rzeczy. Chcemy dodawać 
nowe, przeglądać już dodane (wszystkie na raz jak i pojedynczo),
...
@app.route("/add")
def movie_create():
	# wyświetl formularz, który pozwala dodać film. Zapytaj o tytuł, 
	# reżyserię, rok produkcji i gatunek.
...
def movie_detail(title):
	# dzięki tak sformatowanemu adresowi funkcja movie_detail() 
	# przyjmie z adresu tutył filmu czyli jeśli wejdziesz 
	# na stronę 127.0.0.1:5000/movies/interstellar pod zmienną 
	# lokalną title będziemy mieli string 'interstellar'. 
	# Korzystając z tej zmiennej wyciągnij dany film z bazy i pokaż 
	# wszystkie informacje
	return render_template('movie_detail.html')

@app.route("/movies-list/")
def movie_detail():
	# wyciągnij wszystkie filmy z bazy, pokaż tylko tytuły i rok 
	# produkcji w liscie.
	# jeśli wiesz jak umieść pod tutułem każdego filmu 
	# link do widoku szczegółowego, zrób to
	return render_template('movies_list.html')

@app.route("/movies/<string:title>/edit/")
def movie_edit(title):
	# widok podobny do 'movie_create', z tą różnicą, 
	# że teraz napisujemy istniejący film
